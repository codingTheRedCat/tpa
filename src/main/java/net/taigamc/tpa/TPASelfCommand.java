...
public class TPASelfCommand implements TabExecutor {

    private final TPAPlugin plugin;

    public TPASelfCommand(TPAPlugin plugin) {
        this.plugin = plugin;
        final PluginCommand cmd = plugin.getCommand("tpaself");
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (!sender.hasPermission("tpa.request.self")) {
            sender.sendRichMessage(plugin.config.get().messages.noPermission, TagResolver.builder()
                    .tag("permission", Tag.selfClosingInserting(Component.text("tpa.request.self")))
                    .build());
            return true;
        }
        ...
        try {
            plugin.sendRequest(p, target, TPARequest.TO_SELF);
